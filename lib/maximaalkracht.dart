...
class Maximaalkracht extends StatefulWidget {
  const Maximaalkracht({Key? key}) : super(key: key);

  @override
  _MaximaalkrachtState createState() => _MaximaalkrachtState();
}

class _MaximaalkrachtState extends State<Maximaalkracht> {
  ...
  Future<void> _loadSavedData() async {
    ...
    for (var exercise in exercises) {
      String key = 'maximaalkracht_${exercise['exercise'].toLowerCase().replaceAll(' ', '_')}';
  ...
  Widget buildFilterButton(String category, Color color) {
    ...
    return InkWell(
      onTap: () {
        setState(() {
          selectedCategory = selectedCategory == firstLine ? '' : firstLine;
        });
      },
      child: Container(
  ...
  void _showOverlay(String exerciseId, int repetitions) async {
    ...
    String key = 'maximaalkracht_${normalizedExerciseName}';
    ...
    TextEditingController massaController = TextEditingController(text: (currentValue * 1.167 * 0.75).toStringAsFixed(1));
    TextEditingController uithoudingController = TextEditingController(text: (currentValue * 1.167 * 0.55).toStringAsFixed(1));
    TextEditingController snelheidController = TextEditingController(text: (currentValue * 1.167 * 0.40).toStringAsFixed(1));
    ...
    void updateValue(double change) {
      ...
      massaController.text = (newValue * 1.167 * 0.75).toStringAsFixed(1); 
      uithoudingController.text = (newValue * 1.167 * 0.55).toStringAsFixed(1); 
      snelheidController.text = (newValue * 1.167 * 0.40).toStringAsFixed(1); 
