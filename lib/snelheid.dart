...
class Snelheid extends StatefulWidget {
  const Snelheid({Key? key}) : super(key: key);

  @override
  _SnelheidState createState() => _SnelheidState();
}

class _SnelheidState extends State<Snelheid> {
  ...
  Future<void> _loadSavedData() async {
    ...
    for (var exercise in exercises) {
      String key = 'snelheid_${exercise['exercise'].toLowerCase().replaceAll(' ', '_')}';
...
void _showOverlay(String exerciseId, int repetitions) async {
  ...
  String key = 'snelheid_${normalizedExerciseName}';
  ...
  TextEditingController maximaalkrachtController = TextEditingController(text: (currentValue * 2.5 * 0.85).toStringAsFixed(1));
  TextEditingController massaController = TextEditingController(text: (currentValue * 2.5 * 0.75).toStringAsFixed(1));
  TextEditingController uithoudingController = TextEditingController(text: (currentValue * 2.5 * 0.55).toStringAsFixed(1));
  ...
  void updateValue(double change) {
    ...
    maximaalkrachtController.text = (newValue * 2.5 * 0.85).toStringAsFixed(1); // Update maximaalkrachtController's value
    massaController.text = (newValue * 2.5 * 0.75).toStringAsFixed(1); // Update uithoudingController's value
    uithoudingController.text = (newValue * 2.5 * 0.55).toStringAsFixed(1); // Update snelheidController's value
  ...
  controller.addListener(() {
    ...
    if (newValue != null) {
      ...
      maximaalkrachtController.text = (newValue * 2.5 * 0.85).toStringAsFixed(1);
      massaController.text = (newValue * 2.5 * 0.75).toStringAsFixed(1);
      uithoudingController.text = (newValue * 2.5 * 0.55).toStringAsFixed(1);
    }
  });
  ...
  print('Calculated massa value: ${massaController.text}');
  print('Calculated uithouding value: ${uithoudingController.text}');
   ...
   showDialog(
    ...
    builder: (BuildContext context) {
      ...
      return AlertDialog(
        title: Text('jahoor, sneller dan het licht!'),
        content: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _buildValueText(massaController, Icons.accessibility_new),
            ...
            Row(
              ...
              String massaKey = 'massa_${normalizedExerciseName}';
              ...
              await prefs.setString(massaKey, massaController.text);
              print('Saved value for key $massaKey: ${massaController.text}');
