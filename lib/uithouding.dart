...
class Uithouding extends StatefulWidget {
  const Uithouding({Key? key}) : super(key: key);

  @override
  _UithoudingState createState() => _UithoudingState();
}

class _UithoudingState extends State<Uithouding> {
  ...
  Future<void> _loadSavedData() async {
    ...
    for (var exercise in exercises) {
      String key = 'uithouding_${exercise['exercise'].toLowerCase().replaceAll(' ', '_')}';
...
void _showOverlay(String exerciseId, int repetitions) async {
  ...
  String key = 'uithouding_${normalizedExerciseName}';
  ...
  TextEditingController maximaalkrachtController = TextEditingController(text: (currentValue * 1.8181 * 0.85).toStringAsFixed(1));
  TextEditingController massaController = TextEditingController(text: (currentValue * 1.8181 * 0.55).toStringAsFixed(1));
  TextEditingController snelheidController = TextEditingController(text: (currentValue * 1.8181 * 0.40).toStringAsFixed(1));
  ...
  void updateValue(double change) {
    ...
    maximaalkrachtController.text = (newValue * 1.8181 * 0.85).toStringAsFixed(1); // Update maximaalkrachtController's value
    massaController.text = (newValue * 1.8181 * 0.75).toStringAsFixed(1); // Update uithoudingController's value
    snelheidController.text = (newValue * 1.8181 * 0.40).toStringAsFixed(1); // Update snelheidController's value
  ...
  controller.addListener(() {
    ...
    if (newValue != null) {
      ...
      maximaalkrachtController.text = (newValue * 1.8181 * 0.85).toStringAsFixed(1);
      massaController.text = (newValue * 1.8181 * 0.75).toStringAsFixed(1);
      snelheidController.text = (newValue * 1.8181 * 0.40).toStringAsFixed(1);
    }
  });
